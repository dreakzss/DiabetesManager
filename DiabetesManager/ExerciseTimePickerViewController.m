//
//  ExerciseTimePickerViewController.m
//  DiabetesManager
//
...
@interface ExerciseTimePickerViewController ()

@end

@implementation ExerciseTimePickerViewController
...
- (void)viewDidLoad {
    ...
    exerciseTimeString1 = @"3";
    exerciseTimeString2 = @"5";
    //设置临时初始值
    _exerciseTime = [exerciseTimeString1 stringByAppendingString:exerciseTimeString2];
    //NSLog(@"tempAge:%@",_age);
    
    //存储temp数据
    ReminderModel *sharedManagerRM = [ReminderModel sharedManager];
    sharedManagerRM.tempExceriseTimeString = _exerciseTime;
...
{
    if (component == 0){
        exerciseTimeString1 = [array1 objectAtIndex:row];
        //NSLog(@"array1S : %@", ageString1);
        //[pickerView reloadComponent:0];
    }else{
        exerciseTimeString2 = [array2 objectAtIndex:row];
